...
public class SheetViewController: UIViewController {
    ...
    public func resize(to size: SheetSize, animated: Bool = true) {
        if animated {
            UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0.5, options: .curveEaseOut, animations: { [weak self] in
    ...
    private func setUpChildViewController() {
        ...
        if self.adjustForBottomSafeArea, bottomInset > 0 {
            ...
            let viewSafeArea = UIView(frame: CGRect.zero)
            viewSafeArea.backgroundColor = self.childViewController.view.backgroundColor
            self.containerView.addSubview(viewSafeArea) { subview in
                subview.base.backgroundColor = UIColor.white
...
//        }
    ...
    @objc func panned(_ gesture: UIPanGestureRecognizer) {
        ...
        if gesture.state == .cancelled || gesture.state == .failed {
            ...
            guard finalHeight >= (minHeight / 2) || !dismissOnPan else {
                // Dismiss
                UIView.animate(withDuration: animationDuration, delay: 0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0.5, options: .curveEaseOut, animations: { [weak self] in
                    self?.containerView.transform = CGAffineTransform(translationX: 0, y: self?.containerView.frame.height ?? 0)
                    self?.view.backgroundColor = UIColor.clear
                }, completion: { [weak self] complete in
    ...
    private func adjustForKeyboard(height: CGFloat, from notification: Notification) {
        ...
        UIView.animate(withDuration: duration, delay: 0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0.5, options: animationCurve, animations: {
    ...
    private func height(for size: SheetSize?) -> CGFloat {
        ...
        switch (size) {
            ...
            case .fullScreen:
                ...
                return UIScreen.main.bounds.height - insets.top - 54
            case .halfScreen:
